...
export class ApiRestProductosFotosComponent implements OnInit {
  ...
  get productos_id() {
    return this.formulario.get('productos_id')!;
  }
  get imagen() {
  ...
  enviar() {
    ...
    if (
      extension[largo] == 'png' ||
      extension[largo] == 'jpg' ||
      extension[largo] == 'jpeg'
    ) {
      this.servicio
        .addProductosFotos(
          this.formulario.value.productos_id,
          this.formulario.value.fileSource
        )
        .subscribe({
          next: (data) => {
            Swal.fire({
            ...
            this.router
              .navigate(['/api/productos/fotos/' + this.id])
              .then(() => {
          ...
          error: (error) => {
            Swal.fire({
              icon: 'error',
              title: 'Ops...',
              text: 'Se produjo un error, por favor intentalo de nuevo',
            });
          },
        });
    } else {
